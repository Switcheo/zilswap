...
async function deployNonFungibleToken(
  ...
  const code = (await readFile('./src/ZRC6.scilla')).toString() // ZRC6 token contract
  const init = [
  ...
  console.info(`Deploying ZRC6 token...`)
...
const mintNFT = async () => {
  ...
  // set the spender to be the ark contract
  await callContract(
    seller.key, nft,
    'SetSpender',
    [
...
describe('ARK ExecuteTrade', () => {
  ...
  // test executeTrade for ZRC-6 nft
  test.only('Ark contract should be able to trade a zrc-6 NFT successfully and transfer royalties to the recipient automatically', async () => {
