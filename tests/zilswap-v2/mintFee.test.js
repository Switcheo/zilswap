...
let wZil, token, token0, token1, owner, feeAccount, tx, pool, prevPoolState, newPoolState, router, routerState, prevToken0State, prevToken1State
const BPS = 10000
  ...
  tx = await callContract(
    ...
    [
      param('config', 'Pair ByStr20 Uint128', {
        ...
        "arguments": [`${feeAccount.address}`, `${feeBps}`] // 10%
      })
    ],
    0, false, false
  )
...
describe('zilswap ampPool AddLiquidity, RemoveLiquidty', async () => {
  ...
  test('swap 1', async () => {
    ...
    newPoolState = await pool.getState()
    expect(newPoolState).toEqual(expect.objectContaining({
      "reserve0": `${new BigNumber(prevPoolState.reserve0).plus((new BigNumber(amountIn)).shiftedBy(12)).toString()}`,
    }))
  })
  ...
  test('zilswap ampPool addLiquidity after swap', async () => {
    ...
    expect(newPoolState).toEqual(expect.objectContaining({
      ...
      "balances": {
        "0x0000000000000000000000000000000000000000": `${minimumLiquidity}`,
        [`${feeAccount.address}`]: `${fee.toString()}`,
        [`${owner.address}`]: `${liquidity.plus(prevPoolState.balances[owner.address]).toString()}`,
      },
      "total_supply": `${newTotalSupply.toString()}`
    }))
  ...
  test('zilswap ampPool removeLiquidity after swap', async () => {
    ...
    expect(newPoolState).toEqual(expect.objectContaining({
      ...
      balances: {
        ['0x0000000000000000000000000000000000000000']: `${minimumLiquidity}`,
        [`${feeAccount.address}`]: `${fee.plus(prevPoolState.balances[`${feeAccount.address}`])}`,
        [`${owner.address}`]: '0',
...
describe('zilswap non-amp pool AddLiquidity, RemoveLiquidty', async () => {
  ...
  test('zilswap non-amp pool removeLiquidity after swap', async () => {
    tx = await callContract(
      owner.key, pool,
      'IncreaseAllowance',
      [
    ...
    tx = await callContract(
      ...
      [
        ...
        param('path', 'Pair (ByStr20) (ByStr20)', {
          "constructor": "Pair",
          "argtypes": ["ByStr20", "ByStr20"],
          "arguments": [`${wZil.address.toLowerCase()}`, `${token.address.toLowerCase()}`]
        })
      ],
      0, false, true
    )
...
quote = (amountA, reserveA, reserveB) => {
  return frac(amountA, reserveB, reserveA)
}