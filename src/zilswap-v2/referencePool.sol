    ...
    VolumeTrendRecorder
{
    ...
    constructor()
        public
        ERC20Permit("KyberDMM LP", "DMM-LP", "1")
        VolumeTrendRecorder(0)
    {
        ...
        returns (uint256 liquidity)
    {
        ...
        if (_totalSupply == 0) {
            ...
            liquidity = MathExt.sqrt(amount0.mul(amount1)).sub(
                MINIMUM_LIQUIDITY
            );
            ...
            if (isAmpPool) {
                ...
                _data.vReserve0 = Math.max(
                    data.vReserve0.mul(b) / _totalSupply,
                    _data.reserve0
                );
                _data.vReserve1 = Math.max(
                    data.vReserve1.mul(b) / _totalSupply,
                    _data.reserve1
                );
            }
        }
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        require(
            amount0 > 0 && amount1 > 0,
            "DMM: INSUFFICIENT_LIQUIDITY_BURNED"
        );
    ...
    function swap(
        ...
        ReserveData memory newData;
        {
            ...
            require(
                to != address(_token0) && to != address(_token1),
                "DMM: INVALID_TO"
            );
            ...
            if (isAmpPool) {
                newData.vReserve0 = data.vReserve0.add(newData.reserve0).sub(
                    data.reserve0
                );
                newData.vReserve1 = data.vReserve1.add(newData.reserve1).sub(
                    data.reserve1
                );
            }
        }
    ...
    function sync() external override nonReentrant {
        ...
        if (isAmpPool) {
            ...
            newData.vReserve0 = Math.max(
                data.vReserve0.mul(b) / _totalSupply,
                newData.reserve0
            );
            newData.vReserve1 = Math.max(
                data.vReserve1.mul(b) / _totalSupply,
                newData.reserve1
            );
        }
        ...
        )
    {
        ...
        feeInPrecision = getFinalFee(
            FeeFomula.getFee(rFactorInPrecision),
            _ampBps
        );
    }
    ...
    function verifyBalanceAndUpdateEma(
        ...
        uint256 volume = beforeReserve0.mul(amount1In).div(beforeReserve1).add(
            amount0In
        );
        uint256 rFactorInPrecision = recordNewUpdatedVolume(
            block.number,
            volume
        );
        feeInPrecision = getFinalFee(
        ...
        require(
            balance0Adjusted.mul(balance1Adjusted) >=
                beforeReserve0.mul(beforeReserve1),
            "DMM: K"
        );
    }
    ...
    function _update(bool isAmpPool, ReserveData memory data) internal {
        ...
        if (isAmpPool) {
            assert(
                data.vReserve0 >= data.reserve0 &&
                    data.vReserve1 >= data.reserve1
            ); // never happen
    ...
    function _mintFee(bool isAmpPool, ReserveData memory data)
        internal
        returns (bool feeOn)
    {
            ...
            if (_kLast != 0) {
                ...
                if (_tmp / _vReserve0 == _kLast) {
                    collectedFee0 = _vReserve0.sub(
                        MathExt.sqrt(_tmp.div(_vReserve1))
                    );
                } else {
                    collectedFee0 = _vReserve0.sub(
                ...
                uint256 numerator = totalSupply().mul(collectedFee0).mul(
                    governmentFeeBps
                );
                uint256 denominator = (poolValueInToken0.sub(collectedFee0))
                    .mul(5000);
    ...
    function getReservesData()
        internal
        view
        returns (bool isAmpPool, ReserveData memory data)
    {
    ...
    function getFinalFee(uint256 feeInPrecision, uint32 _ampBps)
        internal
        pure
        returns (uint256)
    {
    ...
    function getK(bool isAmpPool, ReserveData memory data)
        internal
        pure
        returns (uint256)
    {
